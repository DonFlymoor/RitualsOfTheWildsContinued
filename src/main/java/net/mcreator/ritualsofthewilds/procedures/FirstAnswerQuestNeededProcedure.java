...
public class FirstAnswerQuestNeededProcedure {
		...
		if (file.exists() == true) {
			{
				try {
					...
					questneeded = object.get("First Answer Quest Requirement").getAsString();
					thingtodo = object.get("First Answer Quest Requirement State").getAsDouble();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
