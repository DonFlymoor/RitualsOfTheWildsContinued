...
public class GenerateRecipeRedstoneOreProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.RITUALRECIPESREGENERATE.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/Rituals/eng"), File.separator + "21ritual.json");
			try {
			...
			object.addProperty("Ritual Name", "Transmutation Type Ritual: Redstone Ore");
			...
			object.addProperty("Offhand Item ID or Tag", "minecraft:redstone");
			...
			object.addProperty("Result item ID", "minecraft:redstone_ore");
			{
