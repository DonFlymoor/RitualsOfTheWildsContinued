...
public class GenerateRecipeRitualZeroCommandProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.RITUALRECIPESREGENERATE.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/Rituals/eng"), File.separator + "5ritual.json");
			try {
			...
			object.addProperty("Ritual Name", "Block Type Ritual: Light Area");
			object.addProperty("May Spawn In Ritual Scroll", true);
			object.addProperty("Must Read Scroll First", true);
			object.addProperty("Central Block ID or Tag", "rituals_of_the_wilds:moonlight_yr");
			...
			object.addProperty("Result command", "lightuparea");
			object.addProperty("Info about result", "Will make 10 block area bright.");
			{
