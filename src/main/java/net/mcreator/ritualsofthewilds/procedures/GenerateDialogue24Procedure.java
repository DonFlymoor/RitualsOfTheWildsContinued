...
public class GenerateDialogue24Procedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.AUTOREGENDIALOGUES.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/dialogues/eng"), File.separator + "rituals_of_the_wilds_chernobog_23.json");
			try {
			...
			object.addProperty("Text Line", "Hmph. Hunger for purpose \u2014 that is something I respect. But do not confuse purpose with importance.");
			object.addProperty("First Answer Text Line", "\u300B Then set the weight. I\u2019ll carry it.");
			object.addProperty("First Answer Quest Requirement", "chernobog_vex_kill_quest");
			...
			object.addProperty("First Answer Result Quest Give ID", "chernobog_vex_kill_quest");
			object.addProperty("First Answer Result Command Player, World or Entity Executed", 0);
			object.addProperty("First Answer Sound", "");
			object.addProperty("Second Answer Text Line", "\u300B Even ashes can still burn.");
			...
			object.addProperty("Second Answer Result Command", "tellraw @s \"<Chernobog> Spoken like someone who has burned before. We shall see if you still hold the spark.\"");
