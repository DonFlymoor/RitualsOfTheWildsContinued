...
public class ThirdAnswerQuestNeededProcedure {
		...
		if (file.exists() == true) {
			{
				try {
					...
					questneeded = object.get("Third Answer Quest Requirement").getAsString();
					thingtodo = object.get("Third Answer Quest Requirement State").getAsDouble();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
