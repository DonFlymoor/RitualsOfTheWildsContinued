...
public class ThirdAnswerThingProcedure {
		...
		if (file.exists() == true) {
			{
				try {
					...
					soundcommand = object.get("Third Answer Sound").getAsString();
					commandthatexecute = object.get("Third Answer Result Command").getAsString();
					commandtryingplayering = object.get("Third Answer Result Command Player, World or Entity Executed").getAsDouble();
					nextdialogue = object.get("Third Answer Result Dialogue Number").getAsDouble();
					questgive = object.get("Third Answer Result Quest Give ID").getAsString();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
