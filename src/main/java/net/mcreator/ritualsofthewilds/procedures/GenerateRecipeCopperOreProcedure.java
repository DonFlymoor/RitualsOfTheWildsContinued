...
public class GenerateRecipeCopperOreProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.RITUALRECIPESREGENERATE.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/Rituals/eng"), File.separator + "25ritual.json");
			try {
			...
			object.addProperty("Ritual Name", "Transmutation Type Ritual: Copper Ore");
			...
			object.addProperty("Offhand Item ID or Tag", "minecraft:raw_copper");
			...
			object.addProperty("Result item ID", "minecraft:copper_ore");
			{
