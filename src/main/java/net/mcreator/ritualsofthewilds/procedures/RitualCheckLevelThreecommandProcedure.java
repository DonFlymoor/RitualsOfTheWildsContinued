...
public class RitualCheckLevelThreecommandProcedure {
			...
			if (((ForgeRegistries.BLOCKS.getKey((world.getBlockState(BlockPos.containing(x + 2, y, z))).getBlock()).toString()).equals(blockfirst)
							...
							|| (world.getBlockState(BlockPos.containing(x - 3, y, z + 2))).is(BlockTags.create(new ResourceLocation((block16).toLowerCase(java.util.Locale.ENGLISH)))))) {
				{
					try {
						...
						fixedresultitem = object.get("Fixed or Random Executions").getAsBoolean();
						executeasplayer = object.get("Execute as player").getAsBoolean();
						mingive = object.get("Min/Fixed result command execution count").getAsDouble();
						maxgive = object.get("Max command execution count").getAsDouble();
				...
				if (fixedresultitem == false) {
					for (int index0 = 0; index0 < (int) Mth.nextDouble(RandomSource.create(), mingive, maxgive); index0++) {
						if (executeasplayer == true) {
							{
								Entity _ent = entity;
								if (!_ent.level().isClientSide() && _ent.getServer() != null) {
									_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null,
											4, _ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), resultitem);
								}
							}
						} else {
