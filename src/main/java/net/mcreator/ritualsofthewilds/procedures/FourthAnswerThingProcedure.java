...
public class FourthAnswerThingProcedure {
		...
		if (file.exists() == true) {
			{
				try {
					...
					soundcommand = object.get("Fourth Answer Sound").getAsString();
					commandthatexecute = object.get("Fourth Answer Result Command").getAsString();
					commandtryingplayering = object.get("Fourth Answer Result Command Player, World or Entity Executed").getAsDouble();
					nextdialogue = object.get("Fourth Answer Result Dialogue Number").getAsDouble();
					questgive = object.get("Fourth Answer Result Quest Give ID").getAsString();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
