...
public class GenerateRecipeCoalOreProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.RITUALRECIPESREGENERATE.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/Rituals/eng"), File.separator + "23ritual.json");
			try {
			...
			object.addProperty("Ritual Name", "Transmutation Type Ritual: Coal Ore");
			...
			object.addProperty("Result item ID", "minecraft:coal_ore");
			{
