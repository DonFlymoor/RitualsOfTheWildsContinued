...
public class GenerateRecipeRitualZeroProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.RITUALRECIPESREGENERATE.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/Rituals/eng"), File.separator + "1ritual.json");
			try {
			...
			object.addProperty("Ritual Name", "Recipe Type Ritual: Shadow Ink Bottle");
			object.addProperty("May Spawn In Ritual Scroll", true);
			object.addProperty("Must Read Scroll First", true);
			object.addProperty("Central Block ID or Tag", "rituals_of_the_wilds:ash_feoh");
			...
			object.addProperty("Moon Stage", 4);
			...
			object.addProperty("Result item ID", "rituals_of_the_wilds:shadow_ink_bottle");
			{
