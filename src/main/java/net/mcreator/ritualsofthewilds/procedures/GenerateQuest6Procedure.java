...
public class GenerateQuest6Procedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.AUTOREGENQUESTS.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/quests/eng"), File.separator + "chernobog_tied_dried_voidroot_give.json");
			try {
			...
			object.addProperty("Quest name", "Roots of Emptiness");
			object.addProperty("Text Line", "Chernobog demands 8 bundles ofdried Voidroot. Their bitter essence fuels rites no light may touch. Deliver them, and disturb the silence no further.");
