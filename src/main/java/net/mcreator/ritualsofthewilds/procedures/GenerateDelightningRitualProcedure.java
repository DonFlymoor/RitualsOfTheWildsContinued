...
public class GenerateDelightningRitualProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.RITUALRECIPESREGENERATE.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/Rituals/eng"), File.separator + "19ritual.json");
			try {
			...
			object.addProperty("Ritual Name", "Block Type Ritual: Light Extinguish");
			object.addProperty("May Spawn In Ritual Scroll", true);
			object.addProperty("Must Read Scroll First", true);
			object.addProperty("Central Block ID or Tag", "rituals_of_the_wilds:shadow_yr");
			object.addProperty("Offhand Item ID or Tag", "minecraft:torch");
			...
			object.addProperty("Ritual Difficulty Level", 0);
			...
			object.addProperty("Result command", "removelightuparea");
			object.addProperty("Info about result", "Will extinguish the light from another ritual in a 10-block area.");
			{
