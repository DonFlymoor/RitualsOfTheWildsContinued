		...
		if (file.exists() == true) {
			{
				try {
					...
					soundcommand = object.get("First Answer Sound").getAsString();
					commandthatexecute = object.get("First Answer Result Command").getAsString();
					commandtryingplayering = object.get("First Answer Result Command Player, World or Entity Executed").getAsDouble();
					nextdialogue = object.get("First Answer Result Dialogue Number").getAsDouble();
					questgive = object.get("First Answer Result Quest Give ID").getAsString();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
