...
public class GenerateDialogueLeshy2Procedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.AUTOREGENDIALOGUES.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/dialogues/eng"), File.separator + "rituals_of_the_wilds_leshy_2.json");
			try {
			...
			object.addProperty("Text Line", "The wild does not survive by strength alone. It needs hands that carry, not just break.");
			object.addProperty("First Answer Text Line", "\u300B I\u2019ll do it.");
			object.addProperty("First Answer Quest Requirement", "leshy_cow_feed_delivery");
			...
			object.addProperty("First Answer Result Quest Give ID", "leshy_sheep_feed_delivery");
			object.addProperty("First Answer Sound", "");
			object.addProperty("Second Answer Text Line", "\u300B I\u2019m not ready.");
			...
			object.addProperty("Second Answer Result Command", "tellraw @s \"<Leshy> Then be still. The forest will wait.\"");
