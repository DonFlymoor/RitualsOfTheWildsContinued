...
public class GenerateRepairChippedAnvilProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event) {
		...
		if (RitualsOfTheWildsConfigConfiguration.RITUALRECIPESREGENERATE.get() == true) {
			file = new File((FMLPaths.GAMEDIR.get().toString() + "/config/Rituals_of_the_wilds/Rituals/eng"), File.separator + "27ritual.json");
			try {
			...
			object.addProperty("Ritual Name", "Repair Type Ritual: Chipped Anvil");
			object.addProperty("May Spawn In Ritual Scroll", true);
			object.addProperty("Must Read Scroll First", true);
			object.addProperty("Central Block ID or Tag", "minecraft:chipped_anvil");
			...
			object.addProperty("Central Block Replacment ID", "minecraft:anvil");
