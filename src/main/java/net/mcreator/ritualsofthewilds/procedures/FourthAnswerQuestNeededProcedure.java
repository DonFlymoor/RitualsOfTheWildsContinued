...
public class FourthAnswerQuestNeededProcedure {
		...
		if (file.exists() == true) {
			{
				try {
					...
					questneeded = object.get("Fourth Answer Quest Requirement").getAsString();
					thingtodo = object.get("Fourth Answer Quest Requirement State").getAsDouble();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
